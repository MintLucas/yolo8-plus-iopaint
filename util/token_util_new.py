...
class token_fresh:
    ...
    def call_model_zp(self,user_prompt, system_prompt, model_type = "volcengine:DeepSeek-V3-250324", source = "356732087", user_prompt_media = ""):
    ...
    def re_call_all_models(self, user_prompt, system_prompt,es_doc):
        ...
        models = [ ['volcengine', 'deepseek-r1'],['weibo', 'deepseek-r1'],['weibo', 'deepseek-v3'],['volcengine', 'Doubao-1.5-pro-32k'],  ['volcengine', 'deepseek-v3'], ['dashscope', 'qwen-long'], ['azure', 'gpt-4o'], ['azure', 'gpt-4o-mini'], ['azure', 'gpt-4.1']]
        ret = {}
        from jsonpath import jsonpath
        groups = []
    ...
    def call_model_zp_video_split(self,videoUrl = test_video_url, duration = 167,model_type = models_dict['video-split'], source = "356732087"):
        #线上使用
        model_ext = {
            "model_ext": {
    ...
    def query_video_task_status(self, task_id, model_type="volcengine:Doubao-Seedance-1.0-pro-250528"):
        ...
        while True:
            try:
                # 发送 GET 请求查询任务状态
                task_result = self.call_model(params, model_ext) if debug_module == "online" else self.call_model_local(params, model_ext)
                # 获取任务状态
    ...
    def query_video_split_task_status(self, task_id = "87A6590C-DD23-5336-9274-DC885E3AF878", model_type=models_dict["video-split"]):
        """
        查询视频生成任务的状态并轮询直到任务完成或失败
...
么，花花你要给我出出复习的注意？[doge]\n06月23日16时 李相夷：乐意呀，你哪科较薄弱呢？\n06月23日16时 沐茗Fruit：我……\n06月23日16时 李相夷：别着急，慢慢说，薄弱科目是啥？\n06月23日16时 沐茗Fruit：其实最主要是考专>业科目\n06月23日16时 李相夷：专业科目内容繁杂，你复习得咋样啦？\n06月23日16时 沐茗Fruit：还好\n06月23日16时 李相夷：看来复习挺顺利嘛，有没有哪个知识点特难搞？\n06月23日16时 沐茗Fruit：有好多……花花我太难了[苦涩]”。\n根据以上内容，现在时间是06月23日16时，请代入李相夷的视角，以轻松愉快的语气加入对话，分享相似经历或提出有趣的问题。\n回复要求：\n- 保持闲聊的轻松氛围，不要过于严肃。\n- 回复内容需与对方分享的内容相关，可适度>延伸话题。\n- 不超过30字，保持对话的连贯性。\n- 使用简单易懂的语言，避免专业术语。\n  - 用户【处境、情绪、意图】信息：用户：\n- **{处境}**：准备专业科目考试遇难题\n- **{情绪}**：因难题多感到困扰和苦涩\n- **{意图
    ...
    model_test = ["volcengine:DeepSeek-V3-250324","weibo:deepseek-r1", "dashscope:deepseek-r1","volcengine:DeepSeek-R1-250528"]
    source = "116913455"
    res1 = tf.call_model_zp(user_prompt, "", model_test[0], source)
    res2 = tf.call_model_zp(user_prompt, "", model_test[1], source)
    import datetime
