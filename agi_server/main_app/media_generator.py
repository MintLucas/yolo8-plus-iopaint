...
def get_random_style(media_type: str) -> tuple:
    ...
    "boss_health_bar": (
        ...
        MEDIA_TYPE_IMAGE: {
            ...
            "modern_chinese_flat": "Q版卡通现代国风扁平插画，高饱和配色过渡柔和，轮廓线平滑，无厚重阴影",
            "clean_watercolor": "水彩手绘风格，配色明亮柔和，圆乎乎画风，不使用纹理画面干净清爽",
            "woodcut_print": "木版画艺术风格，线条精细，描线清晰，低饱和色调淡雅和谐，突出纹理的色块",
            "c4d_clay_render": "C4D黏土渲染风格，暖调马卡龙色系，无强烈色彩冲突，无手绘笔触，3D渲染的哑光质感",
            "vector_flat": "矢量扁平插画，平涂无纹理的纯色块，简洁的矢量线条，描线清晰，边缘锐利且干净"
        },
        MEDIA_TYPE_VIDEO: {
...
def load_prompt_template(media_type: str) -> str:
    """加载对应媒体类型的提示词模板"""
    template_paths = {
        MEDIA_TYPE_IMAGE_PK: "config/prompt/image_actor_prompt_pk.md",
...
def process_api_questions_generate_media(
    ...
    for idx, item in enumerate(questions, 1):
        ...
        try:
            ...
            role_dict = {"role_a": choose_list[0], "role_b": choose_list[1]}
        ...
        try:
            ...
            if media_type == MEDIA_TYPE_IMAGE_PK:
                style_desc = style_desc.format(**role_dict)    
            ...
            media_path = generate_media_file(
                ...
                media_type=media_type,
                single_file_name = file_pre_dir+datetime.now().strftime("%Y_%m%d_%H%M_%S"),
                para_dict = para_dict
            )
