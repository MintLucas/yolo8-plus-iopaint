...
class servel_help():
    ...
    def send_alert(self, subject, content, mailto = "zhipeng16"):
        try:
            data={
                         'object': '',
                         'service': "service",
                         ...
                         'wechatto': mailto,
                         'sv': '微博AI互动算法',
                         'mailfrom': '微博AI互动算法',#邮件发送名称
                         'subject': subject,
                         'content': content,
                         'html': "1"}
            rt1 = self.api.post("/v1/alert/send",
                     data=data)
            self.log.info(f'send_alert_input: {data}, send_alert_res:{rt1}')
            return "success"
