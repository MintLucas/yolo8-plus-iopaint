...
from baichuan.generation_utils import build_chat_input, TextIterStreamer
...
class BaichuanForCausalLM(BaichuanPreTrainedModel):
    def __init__(self, config, *model_args, **model_kwargs):
        ...
        # fs 增加两层mlp 需要用到nn.Parameter() 会直接把你的这个参数放到优化器里去优化，即加入到可训参数中
        from torch.nn.utils import skip_init
        init_method = skip_init
        init_kwargs = {}
        if self.device is not None:
            init_kwargs["device"] = self.device
        self.lm_head1 = init_method(
            torch.nn.Linear,
            config.vocab_size,
            64,
            bias=False,
            dtype=torch.float, **init_kwargs)
        if self.training:
            torch.nn.init.xavier_uniform_(self.lm_head1.weight)
        self.lm_head2 = init_method(
            torch.nn.Linear,
            64,
            config.vocab_size,
            bias=False,
            dtype=torch.float, **init_kwargs)
        if self.training:
            torch.nn.init.xavier_uniform_(self.lm_head2.weight)
    ...
    def forward(
        ...
        **kwargs,
        ...
        logits0 = self.lm_head(hidden_states)
        # fs 转换到64dim
        logits = self.lm_head2(self.lm_head1(logits0)).contiguous()
    ...
    def chat(self, tokenizer, messages: List[dict], stream=False,
        ...
        if stream:
            ...
            # fs 输出64dim
            logits0 = self.lm_head(hidden_states)
            output_embedding = self.lm_head1(logits0.to(torch.float32)).to(torch.float16).contiguous()
